...
public class PendingOrdersFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        ClientMainActivity.toolbar.setTitle("Pending Orders");

        selectSharedClientAccount();

        rv = v.findViewById(R.id.rv_client_pending_order_card_items);
        tv_empty = v.findViewById(R.id.tv_emptyPendingOrders);
        progressBar = v.findViewById(R.id.progressBar_ClientPendingOrders);
        scrollView = v.findViewById(R.id.scrollViewPindingOrders);
    ...
    private void selectPendingOrders() {
        ...
        db.getReference().child("ClientsReceiptOrders").addChildEventListener(new ChildEventListener() {
            ...
            public void onChildAdded(@NonNull DataSnapshot snapshot, @Nullable String previousChildName) {
                if (snapshot.getValue(ClientOrderReceipt.class) != null) {
                ...
                if (client != null) {
                    for (int i = 0; i < dataList.size(); i++) {
                        if (dataList.get(i).getId().equals(snapshot.getValue(ClientOrderReceipt.class).getId())) {
                            if (snapshot.getValue(ClientOrderReceipt.class).isReceivedByClient()) {
                                dataList.remove(i);
                                adapter.notifyDataSetChanged();
                            }
                        }
